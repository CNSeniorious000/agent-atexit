...
Start sessions with `hermes chat -s defer-cleanup` to preload the skill. The tested Hermes 0.21.3 checkout defers MCP tool descriptions and ignores the server's `anthropic/alwaysLoad` metadata. Apply the compatibility patch below to expose atexit's tools from the first request. Skill preload is still needed; copying the skill alone leaves its use to discovery. The supported preload option is `-s`; this version has no `skills.preload` configuration key.
